...
export function adjustBrightnessContrast(ctx, width, height, brightness, contrast) {
    ...
    for (let i = 0; i < data.length; i += 4) {
        ...
        data[i + 1] = clamp(contrastFactor * (data[i + 1] - 128) + 128 + brightness);
        data[i + 2] = clamp(contrastFactor * (data[i + 2] - 128) + 128 + brightness);
    }
...
export function applySharpen(ctx, width, height) {
    ...
    for (let y = 1; y < height - 1; y++) {
        for (let x = 1; x < width - 1; x++) {
            ...
            sumR += tempData[idx] * 5; sumG += tempData[idx + 1] * 5; sumB += tempData[idx + 2] * 5;

            // up
            idx = getIndex(x, y - 1);
            sumR += tempData[idx] * (-1); sumG += tempData[idx + 1] * (-1); sumB += tempData[idx + 2] * (-1);
