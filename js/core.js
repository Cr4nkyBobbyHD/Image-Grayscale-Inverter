...
import { isGrayscale, convertImageToGrayscale, applyBoxBlur, adjustBrightnessContrast, applyGammaCorrection, applySharpen, applyOtsuThreshold } from './filters.js';
...
export function processAllImages(originalImages, options, updateUI) {
    ...
    originalImages.forEach(imgObj => {
        ...
        img.onload = () => {
            ...
            // Preprocess steps
            updateUI('preprocess', { ctx, width: canvas.width, height: canvas.height, options });
            ...
            for (let y = 0; y < height; y++) {
                for (let x = 0; x < width; x++) {
                    ...
                    originallyGrayscaleMask[y * width + x] = isGrayscale(r, g, b, options.tol) ? 1 : 0;
                }
            }
                    ...
                    if (options.useThreshold) {
                        ...
                        if (options.doTransparent && originallyGrayscaleMask[i / 4] === 1) {
                            if (options.transparencyColor === 'black' && newVal === 0) {
                                data[i + 3] = 0;
                            } else if (options.transparencyColor === 'white' && newVal === 255) {
                                data[i + 3] = 0;
                            } else {
                                data[i + 3] = 255;
                            }
                        ...
                        // Only apply transparency if this pixel was originally grayscale
                        if (options.doTransparent && originallyGrayscaleMask[i / 4] === 1) {
                            if (options.transparencyColor === 'black' && roundedInverted === 0) {
                                data[i + 3] = 0;
                            } else if (options.transparencyColor === 'white' && roundedInverted === 255) {
                                data[i + 3] = 0;
                            } else {
                                data[i + 3] = 255;
                            }
